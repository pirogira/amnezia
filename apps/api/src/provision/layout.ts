...
const SAFE_ABS_PATH = /^\/[a-zA-Z0-9][a-zA-Z0-9._/-]{0,240}$/;
const SAFE_CONTAINER_NAME = /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}$/;

export function assertSafeHostPath(p: string, label: string): string {
  if (p.includes("..") || !SAFE_ABS_PATH.test(p)) {
    throw new Error(`Недопустимый путь ${label}: ${p}`);
  }
  return p;
}
...
export function parseAwgLayoutFromCompose(composeYaml: string, hints?: Partial<AwgHostLayout>): AwgHostLayout {
  ...
  const vol =
    /-\s*(\/[^\s:]+):\/etc\/wireguard/m.exec(composeYaml)?.[1] ??
    /-\s*(\/[^\s:]+):\/etc\/amnezia\/amneziawg/m.exec(composeYaml)?.[1];

...
/**
 * FORWARD / DOCKER-USER — **iptables-legacy** через `nsenter` (как у Docker).
 * **MASQUERADE**: сначала **`iptables` (nf_tables)** через `nsenter` — туда же, что правила Docker;
 * затем то же в **iptables-legacy** (часть ядер реально NATит только legacy). Без `ip nat` в `nft`, CLI `nft insert` не используется.
 */
